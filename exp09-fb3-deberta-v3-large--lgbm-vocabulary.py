...
def get_features(test_loader, model, device):
    ...
    for step, (inputs, targets) in tk0:
        for k, v in inputs.items():
            inputs = collate(inputs)
            inputs[k] = v.to(device)
...
# test_dataset = TrainDataset(CFG, train)
# test_loader = DataLoader(test_dataset, 
#                          batch_size=CFG.batch_size, 
#                          shuffle=False, 
#                          num_workers=CFG.num_workers, pin_memory=True, drop_last=False)

# OUTPUT_DIR = './exp009-fb3-deberta-v3-large-vocabulary/'
...
def run_single_lightgbm(param, train, features, target, fold=0, categorical=[]):
    ...
    train[[f"text_{i}" for i in np.arange(1024)]] = TEXT_FEATURES[fold] #deberta emb : 1024
    
    trn_idx = train[train.fold != fold].index
    val_idx = train[train.fold == fold].index
    print(f'train size : {len(trn_idx)}  valid size : {len(val_idx)}')
    
    model = lgb.LGBMRegressor(**param, n_estimators=300)
    model.fit(train.iloc[trn_idx][features].values, target.iloc[trn_idx].values)

    print(f'Dumping model with pickle... lightgbm_fold{fold}.pkl')
    with open(OUTPUT_DIR+f'lightgbm_fold{fold}.pkl', 'wb') as fout:
        pickle.dump(model, fout)

    oof = train.loc[val_idx,:]
    oof.loc[val_idx, [f'preds_{c}' for c in CFG.target_cols]] = model.predict(train.iloc[val_idx][features])
    score, scores = MCRMSE(target.loc[val_idx,:].values, oof.loc[val_idx, [f'preds_{c}' for c in CFG.target_cols]].values)
    print(f"fold{fold} score: {score:<.5f}, scores: {scores}")

    return oof, val_idx 


def run_kfold_lightgbm(param, train, features, target, n_fold=CFG.n_fold): 
    ...
    for fold in range(n_fold):
        print(f"===== Fold {fold} =====")
        _oof, val_idx = run_single_lightgbm(param, 
                                            train, features, target, 
                                            fold=fold, )
        oof = pd.concat([oof, _oof])
        val_idxes.append(val_idx)
        # break

    val_idxes = np.concatenate(val_idxes)
    score,scores = MCRMSE(target.loc[val_idxes,:].values, oof.loc[val_idxes, [f'preds_{c}' for c in CFG.target_cols]].values)
    print(f"CV score: {score:<.5f},  scores: {scores:}")
    
    return oof, val_idxes

feature_cols = []
target = train[CFG.target_cols]
features = feature_cols + [f"text_{i}" for i in np.arange(1024)] #deberta emb : 768
...
oof,  _ = run_kfold_lightgbm(lgb_param, 
                             train, features, target, 
                             n_fold=CFG.n_fold)
